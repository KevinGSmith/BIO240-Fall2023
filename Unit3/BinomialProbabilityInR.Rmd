...
Up until now, we've been calculating the probability of specific outcomes through two different approaches: by using the rules of probability (and/or and conditional probability) or by simulation in R. Each method produces nearly identical results and we saw, for example, that we have a very consistent distribution of outcomes. For example, almost everyone had the same relative distribution of # of heads out 10 coin flips, and we all calculated similar numbers of false positives and the same positive predictive value for mammograms.
...
for (i in 1:1000){
  results[i] = sum(sample(c(0, 1), 10, replace = TRUE))
}
table(results)
hist(results)
```
...
* pbinom() for calculating "tail" probabilities, which is how much of the probability space is above or below a specific value.
...
In class we'll work through the arguments, but don't forget that you can use ?dbinom() to get some more info about the function.

# Reproducing the coin flip results: probability of specific outcomes
Notice that above we're asking for all possible outcomes (x = 0:10) so we can plot the distribution. If we wanted to know the specific probability of an outcome, however, we could ask just for that outcome:

```{r}
#Probability of 2 heads. Remember, if we put arguments in the correct order, we
#don't need to label  them (but be careful)
dbinom(2, 10, 0.5)
```
This was the same probability that I calculated in class using the rules of probability: (0.5^10) * 45 (different ways of getting 2 heads)
...
ten.flips
```
We can also use : or c() to ask for the total probability in a range of outcomes:
...
Soon when we get into significance testing, we'll be calculating "tail probabilities", which tell us how likely it is that we'd see a result as extreme or more extreme than the result we observed. This is at the heart of significance testing, and when we see a tail probability that's < 0.05, we generally consider it a statistically improbably result. We'll talk MUCH more about this later, but for now we can use this as a way to think about coin-flip results.
